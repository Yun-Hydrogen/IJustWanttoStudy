...
- **✅ 答案模式 (Answer Mode)**
    - **隐藏题目本身，仅保留答案列表**，便于快速核对或导出答案。
    - 自动为答案添加题号标签，按顺序展示在页面底部的“答案列表”中。
...
3. **启用脚本**：打开组卷网试卷详情页，脚本将自动运行，右下角出现悬浮球即代表成功。若未成功运行，请检查是否开启脚本或 Tampermonkey 是否有权限加载用户脚本。
...
2. **非破解工具**：本脚本 **不包含** 任何破解 VIP、非法获取数据或攻击服务器的功能。所有数据均来自用户能够看到的、已获得访问权限的页面。
